...
N <- 2 #number of tracks created

### Main script ----
id <- unique(bird_data.proj$ring)[10] #temporary : we focus on one individual for now
...
for (k in 1:N)
{
  ...
  for (j in 1:n) 
  {
    ...
    for (i in 1:(length(traj_x)-1)) #create a new point betwen point i and i+1 
    {
      ...
      while(length(which(land_out == FALSE)) < 3&n.pts<150) # check that enough points are generated above ocean
      {
        cat("x")
        n.pts <- 2*n.pts
        r <- runif(n.pts,-1,1)
